...
class BEVRandomRotate(nn.Module):
    def __init__(self,
                 prob=0.5,
                 max_rotate_degree=22.5,
                 pc_range=[-50., -50., -5., 50., 50., 3.]
                 ):
        super(BEVRandomRotate, self).__init__()
        self.prob = prob
        self.max_rotate_degree = max_rotate_degree
        self.pc_range = pc_range
...
class BEVRandomFlipTest(nn.Module):
    def __init__(self):
        super(BEVRandomFlipTest, self).__init__()

    def forward(self, feat, gt_bboxes_3d):
        B, C, H, W = feat.shape
        prob = random.uniform(0, 1)
        if prob > self.prob or not self.training:
            return feat
...
class BEVRandomFlipTTA(nn.Module):
    def __init__(self):
        super(BEVRandomFlipTTA, self).__init__()

    def forward(self, feat, img_metas):
        B, C, H, W = feat.shape
        if img_metas[0]['pcd_horizontal_flip']:
            feat = feat.flip(-2)
        if img_metas[0]['pcd_vertical_flip']:
            # flip_type = 'vertical'
            feat = feat.flip(-1)
