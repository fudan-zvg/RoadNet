...
class ShuffleLane(object):
    ...
    """
    def _shuffle_lane(self, results):
        """Pad images according to ``self.size``."""
        gt_lines_coord = np.array(results['gt_lines_coord'])
        gt_lines_label = np.array(results['gt_lines_label'])
        pt_length = gt_lines_coord.shape[0]
        assert pt_length % 3 == 0
        lane_length = pt_length // 3
        gt_lines_coord = gt_lines_coord.reshape(lane_length, 3, 2)
        random_permutation = np.random.permutation(lane_length)
        gt_lines_coord_shuff = gt_lines_coord[random_permutation, :, :]
        gt_lines_coord_shuff = gt_lines_coord_shuff.reshape(pt_length, 2)

        gt_lines_label = gt_lines_label.reshape(lane_length, 3, 1)
        gt_lines_label_shuff = gt_lines_label[random_permutation, :, :]
        gt_lines_label_shuff = gt_lines_label_shuff.reshape(pt_length, 1)

        results['gt_lines_coord'] = gt_lines_coord_shuff
        results['gt_lines_label'] = gt_lines_label_shuff
...
@PIPELINES.register_module()
class NormalizeBEVpretrainImage(object):
    """Normalize the image.
    Added key is "img_norm_cfg".
    Args:
        mean (sequence): Mean values of 3 channels.
    ...
    def __call__(self, results):
        ...
        Returns:
            dict: Normalized results, 'img_norm_cfg' key is added into
                result dict.
        """
        results['bevpretrain_img'] = mmcv.imnormalize(results['bevpretrain_img'], self.mean, self.std, self.to_rgb)
        return results
...
class RoadSegFlip(object):
    ...
    def __call__(self, results):
        ...
        if prob > self.prob:
            ...
            ego2global = ego2global @ rot_mat.T
        results['lidar2ego'] = lidar2ego
        results['ego2global'] = ego2global
        return results
    

@PIPELINES.register_module()
class RoadSegRotateScale(object):
    def __init__(self, prob=0.5,
    ...
    def __call__(self, results):
        ...
        if prob > self.prob:
            ...
            ego2global = ego2global @ aug_matrix.T
            results['lidar2ego'] = lidar2ego
            results['ego2global'] = ego2global
        return results
...
class CenterlineFlip(object):
    ...
    def __call__(self, results):
        ...
        if prob > self.prob:
            ...
            ego2global = ego2global @ np.linalg.inv(rot_mat)
...
class CenterlineRotateScale(object):
    ...
    def __call__(self, results):
        ...
        if prob > self.prob:
            ...
            aug_matrix[:3, :3] = rotation_matrix @ scaling_matrix
            lidar2ego = aug_matrix @ lidar2ego
            ego2global = ego2global @ np.linalg.inv(aug_matrix)
            results['center_lines'] = centerlines
            results['lidar2ego'] = lidar2ego
            results['ego2global'] = ego2global
        return results
